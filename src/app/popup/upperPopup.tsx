...
import { incrementt, decrementt } from '../../lib/features/counter/counterSlice';
...
const UpperPopup: React.FC = () => {
    ...
    const removeProduct = (objj: productType) => {
        // setProducts(products.filter(v => v.id !== obj.id))
        // obj && obj.productName.filter(v => v.id !== obj.id)
        let updatedObj = obj.productName.filter(v => v.id !== objj.id)
        dispatch(decrementt(updatedObj))
